...
public class Lamdba {
	...
	public static List<String> namesStartsWith(List<Student> students, String string) {
		// @formatter:off
		return students.stream().filter(s -> s.getName().startsWith(string.toUpperCase())).map(Student::getName)
	...
	public static Map<Integer, String> failedStudent(List<Student> students, int fromYear, int toYear) {
		...
		// @formatter:off
		return students.stream().filter(s -> s.getGraduationYear() >= fromYear)
				.filter(s -> s.getGraduationYear() <= toYear)
				.filter(
						// putting predicate to invoke its methods like or(),
						// and(), etc,.
						new Predicate<Student>() {
							...
							return s.getEnglish() < pass;
						}) // another form of lambda expression
								.or(s -> s.getMaths() < pass).or(s -> s.getScience() < pass))
	...
	public static List<String> findPassedStudent(List<Student> students) {
		...
		// @formatter:off
		return students.stream().filter(s -> s.getTamil() > pass).filter(s -> s.getEnglish() > pass)
				.filter(s -> s.getMaths() > pass).filter(s -> s.getScience() > pass).map(s -> s.getName())
				.collect(Collectors.toList());
		// @formatter:on
	}
